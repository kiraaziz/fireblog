...
const SearchContent =({Data})=>{
    ...
    return(
        <div className=" w-full flex sm:hidden flex-col items-center h-max p-2"> 
                ...
                Data && Data.items.length !== 0 ? Data.items.map((value) => {
                    return(
                        <a href={`/post/${value.url}`} className="mb-2 mr-2 ml-2  flex sm:flex-row flex-col cursor-pointer shadow-lg rounded-lg h-full sm:w-4/5 w-full bg-gray-900 ">
                            <img alt={`${value.image.desc}`} src={`${value.image.link}`} class=" object-cover sm:w-1/3 h-60 m-2 rounded-lg"/>
                            <div className="sm:w-2/3 w-full h-12/12 p-5 flex flex-col items-left justify-center">
                                <p class="mb-2 text-2xl font-medium f5 text-purple-500 ">
                                    {value.title}
                                </p>
                                <p class="font-light text-lg f4 text-white opacity-50 text-md">
                                    {value.description}
                                </p>
                                <div class="flex flex-wrap items-center mt-4 justify-starts">
                                    {
                                        value.tags &&  value.tags.match(/\b(\w+)\b/g).map((val) => {
                                            return(
                                                <a href={`/tags/${val}`} class="text-md f3 mr-1 mb-1  py-1 px-4 f1 tag-border text-whiterounded-2xl rounded-full text-purple-500 ">
                                                    {val}
                                                </a>
                                            )
                                        })
